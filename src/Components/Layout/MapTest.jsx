import React, { useEffect, useState } from "react";

const MapTest = () => {
  const [petrolStations, setPetrolStations] = useState([]);
  const [bikeServices, setBikeServices] = useState([]);
  const [userLocation, setUserLocation] = useState(null);
  const [mapCenter, setMapCenter] = useState(null);
  const [error, setError] = useState(null);

  // Custom map marker generator
  const createMapMarker = (location, type) => {
    const baseStyle = "absolute transform -translate-x-1/2 -translate-y-1/2 rounded-full";
    const sizeStyle = "w-8 h-8 flex items-center justify-center";
    const textStyle = "text-white font-bold text-sm";

    if (type === 'user') {
      return (
        <div 
          className={`${baseStyle} ${sizeStyle} bg-blue-500 border-4 border-white`}
        >
          <span className={textStyle}>🧑</span>
        </div>
      );
    } else if (type === 'petrol') {
      return (
        <div 
          className={`${baseStyle} ${sizeStyle} bg-green-500 border-4 border-white`}
        >
          <span className={textStyle}>⛽</span>
        </div>
      );
    } else if (type === 'bike') {
      return (
        <div 
          className={`${baseStyle} ${sizeStyle} bg-red-500 border-4 border-white`}
        >
          <span className={textStyle}>🚲</span>
        </div>
      );
    }
  };

  useEffect(() => {
    // Geolocation logic
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          const { latitude, longitude } = position.coords;
          const currentLocation = { 
            lat: latitude, 
            lng: longitude 
          };
          
          setUserLocation(currentLocation);
          setMapCenter(currentLocation);
          
          // Fetch nearby services based on current location
          fetchNearbyServices(latitude, longitude);
        },
        (error) => {
          console.error("Location error:", error);
          // Fallback to a default location
          const defaultLocation = { lat: 40.7128, lng: -74.0060 }; // New York
          setUserLocation(defaultLocation);
          setMapCenter(defaultLocation);
          fetchNearbyServices(defaultLocation.lat, defaultLocation.lng);
          
          // Set error message
          setError("Unable to retrieve your location");
        },
        {
          enableHighAccuracy: true,
          timeout: 5000,
          maximumAge: 0
        }
      );
    } else {
      setError("Geolocation is not supported by this browser");
    }
  }, []);

  const fetchNearbyServices = async (latitude, longitude) => {
    try {
      // Mock API responses - replace with actual API calls
      const petrolStationsData = [
        {
          id: 1,
          name: "Green Energy Petrol Station",
          latitude: latitude + 0.01,
          longitude: longitude + 0.01,
          price: 1.20,
          distance: 2.5,
          fuelTypes: ["Regular", "Premium", "Diesel"],
          availability: "Open 24/7"
        },
        {
          id: 2,
          name: "City Fuel Stop",
          latitude: latitude - 0.01,
          longitude: longitude - 0.01,
          price: 1.15,
          distance: 3.2,
          fuelTypes: ["Regular", "Diesel"],
          availability: "6 AM - 10 PM"
        }
      ];

      const bikeServiceData = [
        {
          id: 1,
          name: "Rapid Bike Repair",
          latitude: latitude + 0.02,
          longitude: longitude + 0.02,
          services: ["Complete Bike Tune-up", "Emergency Repair"],
          specialization: "All Bike Types",
          distance: 4.1,
          rating: 4.5
        },
        {
          id: 2,
          name: "Quick Cycle Solutions",
          latitude: latitude - 0.02,
          longitude: longitude - 0.02,
          services: ["Maintenance", "Custom Modifications"],
          specialization: "Mountain & Road Bikes",
          distance: 5.3,
          rating: 4.2
        }
      ];

      setPetrolStations(petrolStationsData);
      setBikeServices(bikeServiceData);
    } catch (error) {
      console.error("Services fetch error:", error);
    }
  };

  // Render the map with markers
  const renderMap = () => {
    if (!mapCenter) return null;

    return (
      <div 
        className="relative w-full h-full bg-gray-100"
        style={{ 
          backgroundImage: 'linear-gradient(to right, rgba(255,255,255,0.1) 1px, transparent 1px)',
          backgroundSize: '50px 50px'
        }}
      >
        {/* User Location Marker */}
        {userLocation && (
          <div 
            className="absolute z-10"
            style={{
              left: '50%',
              top: '50%',
              transform: 'translate(-50%, -50%)'
            }}
          >
            {createMapMarker(userLocation, 'user')}
          </div>
        )}

        {/* Petrol Stations Markers */}
        {petrolStations.map((station) => (
          <div 
            key={`petrol-${station.id}`}
            className="absolute z-10"
            style={{
              left: `${Math.abs(station.longitude - mapCenter.lng) * 1000}%`,
              top: `${Math.abs(station.latitude - mapCenter.lat) * 1000}%`,
              transform: 'translate(-50%, -50%)'
            }}
          >
            {createMapMarker(station, 'petrol')}
          </div>
        ))}

        {/* Bike Services Markers */}
        {bikeServices.map((service) => (
          <div 
            key={`bike-${service.id}`}
            className="absolute z-10"
            style={{
              left: `${Math.abs(service.longitude - mapCenter.lng) * 1000}%`,
              top: `${Math.abs(service.latitude - mapCenter.lat) * 1000}%`,
              transform: 'translate(-50%, -50%)'
            }}
          >
            {createMapMarker(service, 'bike')}
          </div>
        ))}

        {/* Map Information */}
        <div className="absolute bottom-4 left-4 bg-white p-2 rounded shadow">
          <p>Latitude: {mapCenter.lat.toFixed(4)}</p>
          <p>Longitude: {mapCenter.lng.toFixed(4)}</p>
        </div>
      </div>
    );
  };

  return (
    <div className="flex h-screen">
      {/* Services Sidebar */}
      <div className="w-1/3 p-4 overflow-y-auto bg-gray-100">
        {/* Existing sidebar content from previous implementation */}
        <section className="mb-8">
          <h2 className="text-2xl font-bold mb-4 flex items-center">
            <span className="mr-2 text-blue-600">⛽</span> Petrol Stations
          </h2>
          {petrolStations.map((station) => (
            <div 
              key={station.id} 
              className="bg-white p-4 rounded-lg shadow-md mb-4 hover:shadow-lg transition"
            >
              <div className="flex justify-between items-center mb-2">
                <h3 className="font-semibold text-lg">{station.name}</h3>
                <span className="text-green-600 font-medium">${station.price}/L</span>
              </div>
            </div>
          ))}
        </section>
      </div>

      {/* Map Container */}
      <div className="w-2/3 relative">
        {error ? (
          <div className="w-full h-full flex items-center justify-center bg-red-100">
            <p className="text-red-600 text-xl">{error}</p>
          </div>
        ) : (
          renderMap()
        )}
      </div>
    </div>
  );
};

export default MapTest;